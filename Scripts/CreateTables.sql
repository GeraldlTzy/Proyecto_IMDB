...
CREATE TABLE end_user (
	idUser                     NUMBER(7),
    constraint fk_endUser_systemUser foreign key (idUser) references systemUser(idSystemUser)
);
...
CREATE TABLE administrator (
    ...
    constraint fk_administrator_systemUser foreign key (idAdministrator) references systemUser(idSystemUser)
);
...
CREATE TABLE typeXParticipant (
    idType                     NUMBER(7),
    idParticipant              NUMBER(7),
    CONSTRAINT fk_typeXPart_type FOREIGN KEY (idType) REFERENCES typeOfParticipant(idType),
    CONSTRAINT fk_typeXPart_part FOREIGN KEY (idParticipant) REFERENCES Participant(idParticipant)
);
alter table typeXParticipant
add
constraint pk_typeXParticipant primary key (idType, idParticipant)
using index
tablespace su_ind pctfree 20
storage (initial 10k next 10k pctincrease 0);
/******************************************************************************/
create table Relative (
    ...
    constraint fk_relative_participant foreign key (idParticipant) references participant(idParticipant),
    constraint fk_relative_participant2 foreign key (idRelative) references participant(idParticipant)
);
alter table Relative
add
constraint pk_relative primary key (idParticipant, idRelative)
using index
tablespace su_ind pctfree 20
storage (initial 10k next 10k pctincrease 0);
/******************************************************************************/
...
add
constraint pk_ParticipantXProduct primary key (idPartXprodu)
using index
tablespace su_ind pctfree 20
storage (initial 10k next 10k pctincrease 0);
...
CREATE TABLE CatalogXUser (
	...
	idProduct		            NUMBER(7),
    --isStreamed                  BOOLEAN,
    constraint fk_CatalogXUser_catalog foreign key (idCatalog) references catalog(idCatalog),
    constraint fk_CatalogXUser_user foreign key (idUser) references end_user(idUser),
    constraint fk_CatalogXUser_product foreign key (idProduct) references Product(idProduct)
);

alter table CatalogXUser
add
constraint pk_CatalogXUser primary key (idCatalog, idUser)
using index
tablespace su_ind pctfree 20
storage (initial 10k next 10k pctincrease 0);
/******************************************************************************/
